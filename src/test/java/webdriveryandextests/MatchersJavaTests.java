...
//        }
    ...
    /**
     * Используем матчер decorateMatcherWithWaiter,
     * матчер декоратор should и мачтеры: enabled(), exists(), tagName("div")
     * из библиотеки Yandex-Matchers
     * Матчер everyItem, allOf используем из библиотеки Hamcrest Core
     * <p>
     * В данном примере, проверяется список WebElement-ов,
     * которые удовлетворяют определенным свойствам (См.описание используемых матчеров)
     * Если определенные условия не выполняются,
     * то матчер-декоратор decorateMatcherWithWaiter(..) будет пытаться выполнить проверки
...
//
    ...
    /**
     * Используем матчер-декоратор should и whileWaitingUntil
     * из класса {@link ru.yandex.qatools.matchers.decorators.MatcherDecoratorsBuilder}
     * Матчеры everyItem, anyOf, containsString используем из библиотеки Hamcrest Core
     * Матчер text() используем {@link 8TextMatcher}
     * <p>
     * В данном примере, делаем поиск по тексту Sberbank-Technology
     * и проверяем результат. Каждый {@link WebElement} проверяем,
...
//    }
    ...
    /**
     * Негативный тест. Используем матчер-декоратор should и
     * after {@link ru.yandex.qatools.matchers.decorators.MatcherDecoratorsBuilder}
     * В after(...) передаем {@link ru.yandex.qatools.matchers.decorators.Action},
     * а именно {@link *RefreshPageAction}, т.е
     * находим вебэлемент {@link WebElement}
     * и после обновления страницы проверяем, что он должен быть на странице.
     */
//    @Test
...
//    }
    ...
    /**
     * Проверка, что на странице присутсвует текст "Automation".
     * Работает по принципу matcher.matches(wd.getPageSource()
     * Используется матчер декоратор should из {@link ru.yandex.qatools.matchers.decorators.MatcherDecoratorsBuilder},
     * матчер textOnCurrentPage(...) из {@link •HasTextMatcher},
     * матчер-декоратор is(...) из {@link org.hamcrest.Matchers}
     */
//    @Test
...
//    }
     ...
     * выполняется основной матчер everyItem(exists()),
     * если нет, то everyItem(exists()) - не выполняется
     */
//    @Test
